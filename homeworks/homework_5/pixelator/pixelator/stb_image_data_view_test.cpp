...
TEST_F(StbImageDataViewTest, DefaultConstructor) {
  ...
  ASSERT_THROW(image.at(0, 0), std::out_of_range);
  ASSERT_THROW(image.at(1, 1), std::out_of_range);
...
TEST_F(StbImageDataViewTest, ConstructorWithNonExistingPath) {
  ...
  pixelator::StbImageDataView image(non_existing_path);
...
TEST_F(StbImageDataViewTest, ConstructorWithPathAndImageData) {
  const std::filesystem::path image_path("test_data/test.png");
  ...
  ASSERT_EQ(image.size(), (pixelator::Size{6, 4}));
  ASSERT_EQ(image.rows(), 6);
  ASSERT_EQ(image.cols(), 4);
  ...
  ASSERT_EQ(image.at(2, 3), ftxui::Color(255, 255, 255));
  // Add more test cases as needed
}
...
TEST_F(StbImageDataViewTest, MoveConstructor) {
  const std::filesystem::path image_path("test_data/test.png");
  pixelator::StbImageDataView original_image(image_path);
  ...
  ASSERT_NO_THROW(moved_image.at(0, 0));
  ASSERT_NO_THROW(moved_image.at(1, 1));
  // Add more test cases as needed
}
